...
async fn read_announcement(
    ...
    let announcement = sqlx::query!("SELECT announcement.id, announcement.message, announcement.created_at, character.name FROM announcement INNER JOIN character ON character.id = announcement.character_id WHERE announcement.id = $1", id)
...
async fn list_announcement(
    ...
    let announcements = sqlx::query!("SELECT announcement.id, announcement.message, announcement.created_at, character.name FROM announcement LEFT JOIN character ON character.id = announcement.character_id")
	...
	AnnouncementEntry {
		id: entry.id,
        created_at: entry.created_at,
        created_by: entry.name.unwrap_or("Unknown".to_string()),
        message: entry.message,
     }
	)
    .collect();
...
async fn change_announcement(
    ...
    sqlx::query!(
        "INSERT INTO announcement (id, message, character_id, created_at) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO UPDATE SET message = $2, character_id = $3, created_at = $4",
        input.id,
        input.message,
        account.id,
        now,
